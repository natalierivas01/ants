...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        distance = 0
        bees = curr_loc.bees
        while curr_loc.is_hive == False:
            if bees and self.min_range <= distance <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health <= 0: 
            for i in self.place.bees[:]:
                Ant.reduce_health(i, self.damage)
...
class WallAnt(Ant):
    ...
    def __init__(self): 
        initial_health = 4
        Ant.__init__(self, initial_health)
# The WallAnt class
# END Problem 6
...
# END Problem 12
    ...
    def __init__(self, gamestate):
            # BEGIN Problem 12
            if gamestate.queenz == True: 
                raise DuplicateQueensException()
            else: 
                super().__init__() 
                gamestate.queenz = True 
            # END Problem 12
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 12
        if not self.queenz: 
            self.reduce_health(self.gamestate)
        else: 
